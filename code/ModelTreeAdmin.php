...
use SilverStripe\Admin\ModelTreeAdmin\AddController;
use SilverStripe\Admin\ModelTreeAdmin\EditController;
...
/**
 * Provides functionality for generating a tree-UI admin section for hierarchical data.
 *
 ...
 */
abstract class ModelTreeAdmin extends LeftAndMain implements CurrentPageIdentifier, PermissionProvider, Flushable, MemberCacheFlusher
{
    /**
    ...
    /**
     * The base class for the class hierarchy that this tree admin uses.
     */
    private static string $tree_class = '';
    ...
    private static $url_handlers = [
        ...
        'add//$Action/$ID/$OtherID' => 'handleAdd',
        'edit//$Action/$ID/$OtherID' => 'handleEdit',
        'settings//$Action/$ID/$OtherID' => 'handleSettings',
    ];
    ...
    private static $dependencies = [
        'HintsCache' => '%$' . CacheInterface::class . '.ModelTreeAdmin_SiteTreeHints',
    ];
    ...
    protected function init()
    {
        ...
        if (!$treeClass) {
            throw new LogicException('tree_class must be defined for ' . __CLASS__);
        }
        if (!$treeClass::has_extension(TreeHierarchy::class)) {
            throw new LogicException("tree_class $treeClass must use the " . TreeHierarchy::class . ' extension');
        }

        parent::init();

        // TODO: Move all of these into admin (except page icons which may necessarily be separate)
        ...
        if ($treeClass::has_extension(Versioned::class)) {
    ...
    {
        return $this->config()->get('tree_class');
    }
    ...
    /**
     * Override {@link LeftAndMain} Link to allow blank URL segment for ModelTreeAdmin.
     *
    ...
    public function LinkListView()
    {
        ...
        return $this->LinkWithSearch(static::singleton()->Link('listview'));
    }
    ...
    public function LinkPageEdit($id = null)
    {
        ...
        return $this->LinkWithSearch(
            Controller::join_links(static::singleton()->Link('edit'), 'show', $id)
        );
    }
    ...
    public function LinkPageSettings()
    {
        if ($id = $this->currentPageID()) {
            return $this->LinkWithSearch(
                Controller::join_links(static::singleton()->Link('settings'), 'show', $id)
            );
        } else {
            return null;
        }
    ...
    public function LinkPageHistory()
    {
        if ($id = $this->currentPageID()) {
            return $this->LinkWithSearch(
                Controller::join_links(static::singleton()->Link('history'), 'show', $id)
            );
        } else {
            return null;
        }
    ...
    public function handleAdd(HTTPRequest $reqest)
    {
        return AddController::create($this)->handleRequest($reqest);
    }

    public function handleEdit(HTTPRequest $reqest)
    {
        return EditController::create($this)->handleRequest($reqest);
    }

    public function handleSettings(HTTPRequest $reqest)
    {
        throw new LogicException('not yet implemented - go migrate the CMSPageSettingsController');
    }
    ...
    {
        $link = static::singleton()->Link('add');
    ...
    public function LinkPreview()
    {
        ...
        if ($record && $record->has_extension(Versioned::class)) {
            return $record->Link('?stage=Stage');
        }
    ...
    public function getSiteTreeFor(
        ...
        $checker = $this->getTreeClass()::getPermissionChecker();
        ...
        return $markingSet->renderChildren(
            $this->getTemplatesWithSuffix('_SubTree'),
    ...
    protected function getTreeNodeCustomisations()
    {
        ...
        return function (DataObject $node) use ($rootTitle) {
            return [
                ...
                'Title' => _t(
                    'SilverStripe\\CMS\\Controllers\\CMSMain.PAGETYPE_TITLE',
    ...
    public function updatetreenodes($request)
    {
        ...
        foreach ($ids as $id) {
            ...
            $html = $markingSet->renderChildren(
                $this->getTemplatesWithSuffix('_TreeNode'),
                $this->getTreeNodeCustomisations()
            );
    ...
    {
        // TODO Use searchable fields and ideally searchcontext and even the gridfield search form for this.
        throw new LogicException('Not yet implemented. Need to do this in a clean way that uses $searchable_fields');
    ...
    {
        $pageTypes = [];
        foreach ($this->getTreeClass()::page_type_classes() as $pageTypeClass) {
...
//             }
    ...
    {
        $classes = $this->getTreeClass()::page_type_classes();
    ...
    public function publishall($request)
    {
        ...
        if (isset($this->requestParams['confirm'])) {
            ...
            $start = 0;
            $pages = $this->getTreeClass()::get()->limit("$start,30");
            $count = 0;
            while ($pages) {
                /** @var SiteTree $page */
                foreach ($pages as $page) {
    ...
    public function duplicate($request)
    {
        ...
        return new HTTPResponse(__METHOD__ . " Bad ID: '$id'", 400);
    }
    ...
    {
        if (static::class === self::class) {
            // no-op - only flush on concrete tree admins.
            return;
        }
